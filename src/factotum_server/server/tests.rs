...
fn create_new_server_manager() {
    let server_manager = ServerManager::new(Some("0.0.0.0".to_string()), 8080, "http://a.webhook.com/".to_string(), true, 10_000);
...
fn job_request_can_append_job_args() {
    let server_manager = ServerManager::new(Some("0.0.0.0".to_string()), 8080, "http://dummy.test/".to_string(), true, 10_000);
